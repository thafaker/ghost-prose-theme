...
/* Image zoom (no dependencies)
   ...
   - 2nd click / ESC / backdrop: close
   - Includes keyboard focus + reduced-motion handling
*/
(function () {
  ...
  function getLargeSrc(img) {
    // Prefer srcset biggest candidate if available.
  ...
  function openZoom(img) {
    ...
    const largeSrc = getLargeSrc(img);
    ...
    const closeBtn = overlay.querySelector(".img-zoom-close");
  ...
  function setupImages(root) {
    ...
    imgs.forEach(img => {
      ...
      img.setAttribute("aria-label", (img.getAttribute("alt") || "Bild") + " – klicken zum Vergrößern");
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ghost can inject content via members/portal etc. — observe new images.
